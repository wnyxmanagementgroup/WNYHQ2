...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz6oZhMmP8boS9ABuDkYdWrLyZlSkWp9JuDMuBHTex7x9YSDqv6IzMvoZ5BIeeWIf0Yig/exec";
...
function initializeUserSession(user) {
    ...
    showMainApp(); // สั่งให้แสดงผล

    // ✅ หน่วงเวลา 100ms เพื่อรอให้เบราว์เซอร์วาดหน้า #main-app ให้เสร็จก่อน
    setTimeout(() => {
        try {
            ...
            console.error("Error setting up App Event Listeners:", e);
            showAlert("ข้อผิดพลาดร้ายแรง", "ไม่สามารถเริ่มการทำงานของแอปได้ กรุณาลอง Hard Reload (Ctrl+Shift+R)");
        }
    }, 100); 
}
...
function setupAuthEventListeners() {
    ...
    // General Modal close logic (for clicks outside)
    ...
    // Close buttons for general modals (Alert, Confirm)
    ...
    // Global error handlers (can load at start)
    window.addEventListener('error', (event) => {
        // ✅ เพิ่มการตรวจสอบว่า event.error มีอยู่จริงหรือไม่
        if (!event.error) {
            console.error('Global error: Unknown (event.error is null)');
            return;
        }
...
function setupAppEventListeners() {
    ...
    // App-specific Modals (Close buttons)
    ...
    // App-specific Modals (Forms & Actions)
    ...
    // ✅ อัปเดต: เรียกใช้ฟังก์ชันติดตั้งระบบเลือกพาหนะแบบหลายตัวเลือก
    ...
    // Admin (with null checks)
    const addUserBtn = document.getElementById('add-user-button');
    if (addUserBtn) addUserBtn.addEventListener('click', openAddUserModal);

    const downloadUserTemplateBtn = document.getElementById('download-user-template-button');
    if (downloadUserTemplateBtn) downloadUserTemplateBtn.addEventListener('click', downloadUserTemplate);

    const importUsersBtn = document.getElementById('import-users-button');
    if (importUsersBtn) importUsersBtn.addEventListener('click', () => {
    ...
    const adminMemosList = document.getElementById('admin-memos-list');
    if (adminMemosList) adminMemosList.addEventListener('click', handleAdminMemoClick);
    
    // Admin Page Tabs (with null checks)
...
function handleAdminMemoClick(e) {
    const button = e.target.closest('.admin-memo-button[data-action="manage"]');
    if (button) {
        const memoId = button.dataset.memoId;
        openAdminMemoAction(memoId);
    }
}
...
function renderAdminRequestsList(requests) {
    ...
    // ✅ ใช้ Event Delegation สำหรับการจัดการคลิก
    setupAdminRequestsEventDelegation();
    
    console.log(`✅ Render สำเร็จ: ${requests.length} คำขอ, Event listeners ถูกตั้งค่าแล้ว`);
}

// ✅ ฟังก์ชันตั้งค่า Event Delegation
function setupAdminRequestsEventDelegation() {
    const container = document.getElementById('admin-requests-list');
    
    if (!container) {
        console.error('❌ ไม่พบ container admin-requests-list');
        return;
    }

    // ลบ event listener เดิมก่อนเพิ่มใหม่ (ป้องกัน duplication)
    container.removeEventListener('click', handleAdminRequestsClick);
    
    // เพิ่ม event listener ใหม่
    container.addEventListener('click', handleAdminRequestsClick);
    
    console.log('✅ Event delegation ถูกตั้งค่าสำหรับ admin requests');
}

// ✅ ฟังก์ชันจัดการการคลิกแบบรวมศูนย์
function handleAdminRequestsClick(e) {
    const target = e.target;
    
    // ตรวจสอบปุ่มออกหนังสือส่ง
    if (target.classList.contains('dispatch-button')) {
        const requestId = target.getAttribute('data-request-id');
        handleDispatchButtonClick(requestId, target);
        return;
    }
    
    // ตรวจสอบปุ่มออกคำสั่ง
    if (target.classList.contains('command-button')) {
        const requestId = target.getAttribute('data-request-id');
        handleCommandButtonClick(requestId, target);
        return;
    }
    
    // กรณีคลิกบน element ภายในปุ่ม (เช่น icon, span)
    const dispatchButton = target.closest('.dispatch-button');
    if (dispatchButton) {
        const requestId = dispatchButton.getAttribute('data-request-id');
        handleDispatchButtonClick(requestId, dispatchButton);
        return;
    }
    
    const commandButton = target.closest('.command-button');
    if (commandButton) {
        const requestId = commandButton.getAttribute('data-request-id');
        handleCommandButtonClick(requestId, commandButton);
        return;
    }
}

// ✅ ฟังก์ชันจัดการการคลิกปุ่มออกหนังสือส่ง
function handleDispatchButtonClick(requestId, button) {
    console.log("🖱️ Dispatch button clicked for:", requestId);
    
    if (!requestId) {
        console.error('❌ ไม่พบ requestId ในปุ่มออกหนังสือส่ง');
        showAlert('ผิดพลาด', 'ไม่พบรหัสคำขอ');
        return;
    }
    
    // แสดง loading state บนปุ่มชั่วคราว
    const originalText = button.innerHTML;
    button.innerHTML = '⏳ กำลังเปิด...';
    button.disabled = true;
    ...
    // เปิด modal หนังสือส่ง
    openDispatchModal(requestId);
}

// ✅ ฟังก์ชันจัดการการคลิกปุ่มออกคำสั่ง
function handleCommandButtonClick(requestId, button) {
    console.log("🖱️ Command button clicked for:", requestId);
    
    if (!requestId) {
        console.error('❌ ไม่พบ requestId ในปุ่มออกคำสั่ง');
        showAlert('ผิดพลาด', 'ไม่พบรหัสคำขอ');
        return;
    }
    ...
    // เปิด modal อนุมัติคำสั่ง
    openCommandApproval(requestId);
}

// ✅ ฟังก์ชันตรวจสอบสถานะปุ่ม (สำหรับ debug)
function checkAdminButtonsStatus() {
    console.log("🔍 Admin Buttons Status Check:");
    
    const dispatchButtons = document.querySelectorAll('.dispatch-button');
    const commandButtons = document.querySelectorAll('.command-button');
    const container = document.getElementById('admin-requests-list');
    
    console.log(`📦 Dispatch buttons: ${dispatchButtons.length}`);
    console.log(`📋 Command buttons: ${commandButtons.length}`);
    console.log(`📁 Container:`, container);
    
    dispatchButtons.forEach((btn, index) => {
        console.log(`Dispatch button ${index}:`, {
    ...
    commandButtons.forEach((btn, index) => {
        console.log(`Command button ${index}:`, {
            requestId: btn.getAttribute('data-request-id'),
            text: btn.textContent,
            disabled: btn.disabled
        });
...
// ✅ ทำให้ฟังก์ชันตรวจสอบสามารถเรียกใช้จาก console ได้
window.checkAdminButtonsStatus = checkAdminButtonsStatus;
            ...
            <div class="border rounded-lg p-4 bg-white">
                <div class="flex justify-between items-start">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <button data-action="manage" data-memo-id="${memo.id}" class="btn bg-green-500 text-white btn-sm admin-memo-button">
...
async function handleCommandApproval(e) {
    ...
    try {
        // ใน app.js, ฟังก์ชัน handleCommandApproval(e)
        const result = await apiCall('POST', 'approveCommand', { // <-- แก้ไขตรงนี้
        requestId: requestId,
        templateType: commandType, // <-- payload key ควรตรงกับ Code.gs (payload.templateType)
